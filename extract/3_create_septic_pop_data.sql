...
BEGIN
    EXECUTE 
    'UPDATE '||update_table_name||' AS t1 SET v'||column_number||'=(COALESCE(v'||column_number||',(SELECT value FROM '||data_table_name||
' AS t2 WHERE t1.hadm_id=t2.hadm_id AND t2.sepsis_lab_id='||column_number|| ' AND t2.charttime<=(t1.fmt + interval ''48 hours'')AND t2.charttime>=(t1.fmt) ORDER BY ABS((EXTRACT(DAY FROM t2.charttime-t1.time))*24*60 + (EXTRACT(HOUR FROM t2.charttime-t1.time))*60 + (EXTRACT(MINUTE FROM t2.charttime-t1.time))) LIMIT 1)))'
    ;
    RETURN;
END;
$ret$ LANGUAGE plpgsql;
